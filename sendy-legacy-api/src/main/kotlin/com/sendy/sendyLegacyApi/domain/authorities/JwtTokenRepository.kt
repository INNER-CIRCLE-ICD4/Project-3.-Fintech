...
interface JwtTokenRepository : JpaRepository<JwtTokenEntity, Long> {
     ...
     */
    fun findByTokenHashAndStatusIn(
        tokenHash: String,
        statuses: List<TokenStatus>,
    ): Optional<JwtTokenEntity>
     ...
     * 사용자의 특정 타입 활성 토큰 찾기
     */
    fun findByUserIdAndTokenTypeAndStatus(
        userId: Long,
        tokenType: TokenType,
        status: TokenStatus,
    ): List<JwtTokenEntity>
    ...
    @Modifying
    @Query(
        "UPDATE JwtTokenEntity j SET j.status = 'PENDING_LOGOUT', j.updatedAt = :updatedAt WHERE j.userId = :userId AND j.status = 'ACTIVE'",
    )
    fun setPendingLogoutByUserId(
        userId: Long,
        updatedAt: LocalDateTime,
    )
    ...
    @Modifying
    @Query(
        "UPDATE JwtTokenEntity j SET j.status = 'REVOKED', j.updatedAt = :updatedAt WHERE j.userId = :userId AND j.tokenType = :tokenType AND j.status != 'REVOKED'",
    )
    ...
    @Modifying
    @Query(
        "UPDATE JwtTokenEntity j SET j.status = 'REVOKED', j.updatedAt = :updatedAt WHERE j.deviceId = :deviceId AND j.status != 'REVOKED'",
    )
    fun revokeTokensByDeviceId(
        deviceId: Long,
        updatedAt: LocalDateTime,
    )
