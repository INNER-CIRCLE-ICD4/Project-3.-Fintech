    ...
    override fun handle(event: EmailVerificationSentEvent) {
        val notification =
            notificationEventService.createNotification(
                ...
                message = "${event.username}님, ${event.email}로 인증 링크를 발송했습니다. 이메일을 확인해주세요!.",
                notificationId = System.currentTimeMillis(),
                createdAt = Instant.now(),
            )
        log.info(
            "이메일 인증 알람 생성 완료! 사용자:{}, 알림ID: {}",
            event.username,
            notification.notificationId,
        )
    }

    override fun supports(eventType: String): Boolean = eventType == "USER_VERIFICATION"
}
